...
import { CuratedWordData, WordDataResponse, WordDataNotFound } from '../types/types'

function useCurateWordData(wordDataResponse: WordDataResponse[] | WordDataNotFound | undefined): CuratedWordData | undefined {
	const [curatedWordData, setCuratedWordData] = useState<CuratedWordData>()

	useEffect(() => {
		if (Array.isArray(wordDataResponse)) { // If the data is an array, it means that the word was found
